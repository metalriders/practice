...
vector_cell getNeigbors(grid_t &grid, pair_int_int bounds, pair_int_int coords)
{
	...
	if (up >= 0)
	{
		neigbors.push_back(Cell{ DirectionType::up, &grid[up][coords.second] });
	}
	if (down <= bounds.first - 1)
	{
		neigbors.push_back(Cell{ DirectionType::down, &grid[down][coords.second] });
	}
	if (left >= 0)
	{
		neigbors.push_back(Cell{ DirectionType::left, &grid[coords.first][left] });
	}
	if (right <= bounds.second - 1)
	{
		neigbors.push_back(Cell{ DirectionType::right, &grid[coords.first][right] });
	}
...
int sumInPerimeter(grid_t grid, int p) {
	...
	{	
		...
		path.push_back(Cell{ DirectionType::undef, item });
		sum = *path[0].data_ptr;
		perimeter = 4;
		...
		while (true)
		{
			...
			else if (perimeter < p && path.size() < p / 2)
			{
...
int main(int argc[], char** arcgv)
{
	...
	// Test case #15
	grid = { { 1, -1, 1, -1 }, { 1,0,5,10 }, { 1,0,0,4 } };		// needs to be 20 :(
	p = 10;
	std::cout << "Max sum is: " << sumInPerimeter(grid, p) << "\n\n";
